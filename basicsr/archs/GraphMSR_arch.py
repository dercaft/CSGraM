...
from MSR_kit import flex, cossim, local_sampling, global_sampling
...
class GraphMSR_Grapher(nn.Module):
...
class Simple_Grapher(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(Simple_Grapher, self).__init__()
        self.conv = nn.Conv1d(input_dim, output_dim, kernel_size=1, bias=False)
        self.relu = nn.ReLU()
...
class GAM(nn.Module):
    ...
    def __init__(self, in_channels, patch_size, embed_dim):
        ...
        self.inv_proj = nn.Linear(embed_dim, in_channels * patch_size * patch_size)
        self.patch_unfold = nn.Unfold(kernel_size=patch_size, stride=patch_size)

        self.f_cp = nn.Linear(embed_dim, embed_dim)
        self.sigmoid = nn.Sigmoid()

        self.fglg_fc1 = nn.Linear(1, embed_dim)
        self.leaky_relu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        self.fglg_fc2 = nn.Linear(embed_dim, embed_dim)

        self.F_transform = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self, x):
        ...
        patches = self.patch_unfold(x)  # (B, C*p*p, L)
        patches = patches.transpose(1, 2)

        graph_features = self.proj(patches)

        Ap = self.sigmoid(self.f_cp(graph_features))  # (B, L, embed_dim)

        m = torch.mean(graph_features, dim=-1, keepdim=True)
        m_fglg = self.fglg_fc2(self.leaky_relu(self.fglg_fc1(m)))

        Af = self.sigmoid(Ap * m_fglg)

        G_transformed = self.F_transform(graph_features)

        graph_features_out = Af * G_transformed  # (B, L, embed_dim)

        patch_out = self.inv_proj(graph_features_out)  # (B, L, C*p*p)
 
        patch_out = patch_out.transpose(1, 2)

        fold = nn.Fold(output_size=(H, W), kernel_size=p, stride=p)
        out = fold(patch_out)  # (B, C, H, W)
...
class MultimodalFusion(nn.Module):
    def __init__(self, dim, text_dim=512, num_heads=8):
        super().__init__()
    ...
    def forward(self, img_feats, text_embeddings):
        """
        img_feats: [B, H*W, C]
        text_embeddings: [B, seq_len, text_dim]
        """

        text_feats = self.text_proj(text_embeddings.mean(1))  # [B, C]
        text_feats = text_feats.unsqueeze(1)  # [B, 1, C]
        ...
        # 残差连接
        fused_feats = self.norm(img_feats + attn_output)
        fused_feats = fused_feats + self.mlp(fused_feats)
        return fused_feats


class GACB(nn.Module):
    ...
    def __init__(self,
        ...
        super(GACB, self).__init__()
        self.kwargs = kwargs

        self.dim = dim
        self.input_resolution = input_resolution

        self.mllm_fusion = MultimodalFusion(embed_dim, mllm_dim)
        self.semantic_proj = nn.Linear(mllm_dim, embed_dim)
        self.gam = GAM(in_channels, 4, embed_dim)
    ...
    def forward(self, x, x_size, prev_graph=None, text_embeddings=None):
        graph = self.calc_graph(x, x_size) if self.graph_flag else prev_graph
        res = self.residual_group(x, x_size, graph)
        if text_embeddings is not None:
            B, C, H, W = x.shape
            img_feats = res.view(B, C, H * W).permute(0, 2, 1)  # [B, HW, C]
            fused_feats = self.mllm_fusion(img_feats, text_embeddings)
            res = res + 0.5 * fused_feats.permute(0, 2, 1).view(B, C, H, W)

        return self.patch_embed(self.gam(self.conv(self.patch_unembed(res, x_size)))) + x, graph
